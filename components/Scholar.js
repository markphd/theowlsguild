  ...
  useEffect(() => {
    fetch(
      ...
      .then(async (response) => {
        ...
        info.days = moment
          .utc(info.last_claimed_item_at * 1000).local()
          .add("days", 14)
      ...
      fetch(`https://explorer.roninchain.com/api/tokentxs?addr=${player.address}&from=0&size=1&token=ERC20`)
      .then(async (response) => {
  ...
  return (
    <tr key={Math.random(23)}>
      <td className="name">
        ...
        <a className="link" href={`https://explorer.roninchain.com/address/${player.address}`}>
          {player.alias} ↗
        </a>
      </td>
